...
{
    adore::if_ROS::TrajectoryPlannerALCNode lcn;
    ros::init(argc,argv,"adore_trajectory_planner_alc_node");  // TODO ros init twice, once here, once in baseapp
    ros::NodeHandle nh("~");
    std::string direction;
    if(!nh.getParam("direction",direction)||(direction.compare("left")!=0 && direction.compare("right")!=0))
    {
        ROS_DEBUG("No direction given to adore_trajectory_planner_alc_node node, terminating.");
    ...
    {
        ROS_DEBUG("No name given to adore_trajectory_planner_alc_node node, terminating.");
    ...
    {
        ROS_DEBUG("No id given to adore_trajectory_planner_alc_node node, terminating.");
    ...
    {
        ROS_DEBUG("No speed scale given to adore_trajectory_planner_alc_node node.");
    ...
    {
        ROS_DEBUG("No lateral grid index given to adore_trajectory_planner_alc_node node.");
    ...
    {
        ROS_DEBUG("No const penalty given to adore_trajectory_planner_alc_node node.");
    ...
    {
        ROS_DEBUG("No em_continue_active penalty given to adore_trajectory_planner_alc_node node.");
    ...
    if(!nh.getParam("em_cancel_active",em_continue_active))
    {
