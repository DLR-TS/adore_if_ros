...
namespace adore
{
    namespace if_ROS
    {
        ...
        /**
         * ROS specific implementation of AFeed for VehicleMotionState9d.
         * Reads nav_msgs::Odometry, std_msgs::Float32 (steering angle), std_msgs::Float32 (acceleration) and converts to adore::fun::VehicleMotionState9d
         */
        class MotionStateFeed:public adore::mad::AFeed<adore::fun::VehicleMotionState9d>
        {
            private:
            ros::Subscriber odom_subscriber_;
            ros::Subscriber steering_subscriber_;
            ...
            void receive_odom(nav_msgs::OdometryConstPtr msg)
            {
                adore::fun::VehicleMotionState9d data;
                data.setTime(msg->header.stamp.toSec());
                data.setX(msg->pose.pose.position.x);
                data.setY(msg->pose.pose.position.y);
                data.setZ(msg->pose.pose.position.z);
                QuaternionConverter qc;
                data.setPSI(qc.quaternionToHeading(msg->pose.pose));
                data.setvx(msg->twist.twist.linear.x);
                data.setvy(msg->twist.twist.linear.y);
                data.setOmega(msg->twist.twist.angular.z);
                data.setAx(ax_);
                data.setDelta(delta_);
            ...
            void receive_steering(std_msgs::Float32ConstPtr msg)
            {
                ...
                delta_ = msg->data /  params_->get_steeringRatio();
            }
            void receive_acceleration(std_msgs::Float32ConstPtr msg)
            {
                ax_ = msg->data;
            ...
            public:
            MotionStateFeed(ros::NodeHandle* n,
                                const std::string odom_topic,
                                const std::string accelerationTopic,
                                const std::string steeringAngleTopic,
                                int qsize)
            {
                ...
                delta_ = 0.0;
                n->param("/tcp_no_delay", no_delay, false);
                odom_subscriber_ = n->subscribe(odom_topic,qsize,&MotionStateFeed::receive_odom,this,ros::TransportHints().tcpNoDelay(no_delay));
                acceleration_subscriber_ = n->subscribe(accelerationTopic,qsize,&MotionStateFeed::receive_acceleration,this,ros::TransportHints().tcpNoDelay(no_delay));
                steering_subscriber_ = n->subscribe(steeringAngleTopic,qsize,&MotionStateFeed::receive_steering,this,ros::TransportHints().tcpNoDelay(no_delay));
                params_ = nullptr;
            }
            /**
        ...
        class MotionStateWriter:public adore::mad::AWriter<adore::fun::VehicleMotionState9d>
        {
            ...
            MotionStateWriter(ros::NodeHandle* n,const std::string& frame_id,const std::string& odom_topic,const std::string& steering_topic,const std::string& acceleration_topic,int qsize)
                : publisher_steering_initialized_(false), publisher_acceleration_initialized_(false), frame_id_(frame_id)
            {
                publisher_odom_= n->advertise<nav_msgs::Odometry>(odom_topic,qsize);
                if(steering_topic.size()>0)
                {  
                    publisher_steering_ = n->advertise<std_msgs::Float32>(steering_topic,qsize);
                    publisher_steering_initialized_ = true;
                }
                if(acceleration_topic.size()>0)
                {
                    publisher_acceleration_ = n->advertise<std_msgs::Float32>(acceleration_topic,qsize);
                    publisher_acceleration_initialized_ = true;
                }
                params_=nullptr;
            }
            virtual bool canWriteMore() const override
            ...
            virtual void write(const adore::fun::VehicleMotionState9d& value)override
            {
                ...
                if(publisher_steering_initialized_)publisher_steering_.publish(delta);
                std_msgs::Float32 acceleration;
                acceleration.data = value.getAx();
                if(publisher_acceleration_initialized_)publisher_acceleration_.publish(acceleration);
